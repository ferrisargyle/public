	javascript: (function() {
		...
		var folderHeightSelector = '.a-Fa-U .a-U-H .a-pa-P-d';
		...
		/* use allCSS sample function to find all rules: https://developer.mozilla.org/en-US/docs/Web/API/StyleSheetList */

		var defaultFolderHeightRule = getCSSRule(folderHeightSelector);
		if (defaultFolderHeightRule) {
			defaultFolderHeight = defaultFolderHeightRule.style.getPropertyValue('height');
		}
				...
				for (var j in elementArr) {
					...
					if (element.classList && !element.classList.contains(myDriveExclusionClass)) {
						if (element.classList.contains('new_folder_height')) {
							...
							/* the following isn't needed anymore since grab row height directly from CSS
							if (initialize) {
		...
		*/
		...
		toggleProperty('height', folderHeightArr, defaultFolderHeight, newFolderHeight);
